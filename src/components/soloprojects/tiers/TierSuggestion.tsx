...
import { SoloProjectFields, SoloProjectTier } from '@/types/SoloProjectTypes';
...
const TierSuggestion = ({
  ...
  onSuccess: (field: keyof SoloProjectFields, value: string) => void;
}) => {
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          <DropdownMenuRadioGroup
            onValueChange={(value: string) => handleTierChange(value as SoloProjectTier)}
          >
            {dropDownValues.map((v) => (
              <DropdownMenuRadioItem value={v.value} key={v.value}>
                {v.text}
              </DropdownMenuRadioItem>
            ))}
          </DropdownMenuRadioGroup>
