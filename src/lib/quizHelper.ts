...
export const mappedQuestions = (fields: SoloProjectFields, questions: Answer[]) => {
  return questions.map((q) => {
    return {
      ...
      userAnswer: fields[q.questionNumber as keyof SoloProjectFields],
      isAnswerCorrect: isAnswerCorrect(
        fields[q.questionNumber as keyof SoloProjectFields],
        q.answer
      ),
    };
  });
...
export const generateTextForWrongAnswers = (questions: MappedQuestionAndAnswer[]) => {
  return questions
    .filter((q) => q.isAnswerCorrect !== true)
    .map((q) => {
      return `${q.question}\n\n${q.options}`;
    })
    .join('\n\n---\n\n');
